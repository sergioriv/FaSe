...
$objPHPExcel->getProperties()->setCreator("FaSe | MantizTechnology") // Nombre del autor
    ...
    ->setTitle("Reporte arqueo") // Titulo
    ->setSubject("Reporte arqueo") //Asunto
    ->setDescription("Reporte arqueo") //Descripción
    ->setKeywords("reporte arqueo") //Etiquetas
    ->setCategory("reporte excel arqueo"); //Categorias
...
//Se agregan los datos de los alumnos
 ...
 while ($fila = $formasPagoSql->fetch_assoc()) {
    ...
    $abonosFP = $con->query("SELECT SUM(ab_abono) AS sumaFP FROM abonos WHERE ab_idClinica='$sessionClinica' AND ab_idFormaPago='$fila[IDFormaPago]' AND ab_estado=1 AND YEAR(ab_fechaCreacion)='$fechaAR[0]' AND MONTH(ab_fechaCreacion)='$fechaAR[1]' AND DAY(ab_fechaCreacion)='$fechaAR[2]'")->fetch_assoc();
    if($abonosFP['sumaFP'] == null){ $sumaFP = 0; }
        else { $sumaFP = $abonosFP['sumaFP']; }

     $objPHPExcel->setActiveSheetIndex(0)
         ->setCellValue('A'.$i, $fila['fp_nombre'])
         ->setCellValue('B'.$i, $sumaFP);
     $i++;
 }
