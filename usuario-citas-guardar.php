...
if(!$_POST['id']){
	...
	$query = $con->query("INSERT INTO usuarioscitas SET uc_idClinica='$sessionClinica', uc_idUsuario='$sessionIDUsuario', uc_idSucursal='$sucursal', uc_nombres='$nombre', uc_correo='$correo', uc_enviarCorreo='$enviarAlertas', uc_estado='1', uc_fechaCreacion='$fechaHoy'");
	$id = $con->insert_id;

	if($query){ $_SESSION['consultoriosExito']=2;

		$con->query("INSERT INTO usuarios SET us_idClinica='$sessionClinica', us_idRol='5', us_id='$id', us_nombre='$nombre', us_correo='$correo', us_password='$passwordUsuarios', us_estado='1', us_fechaCreacion='$fechaHoy'");
			...
			<table border='0' style='border-collapse: collapse; width: 100%; font-family: Helvetica, Arial, sans-serif; color: black; font-size: 15px;'>
			  ...
			  <tr>
				<td>
					...
					<p><b>".strtoupper($clinicaRow['cl_nombre'])."</b> ahora utiliza la platafoma de Gestion de Citas <b>FaSe</b>, en ella podrás llevar el control de citas.</p>
					<p><br></p>
					<p>Tus datos de acceso son:</p>
					<p>Correo: ".$correo."<br>Contraseña: ".$passwordUser."<br>Rol: Citas</p>
					<p></p>
	...
	$id = $_POST['id'];
	$query = $con->query("UPDATE usuarioscitas SET uc_idSucursal='$sucursal', uc_nombres='$nombre', uc_correo='$correo', uc_enviarCorreo='$enviarAlertas' WHERE IDUserCitas = '$id'");

	if($query){ $_SESSION['consultoriosExito']=3;

		$con->query("UPDATE usuarios SET us_nombre='$nombre', us_correo='$correo' WHERE us_id = '$id' AND us_idRol='5'");
