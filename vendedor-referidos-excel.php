...
$id = $_GET['id'];
    ...
    $vnReferidoRangoDe = str_replace("-", "", $_GET['de']);
    $vnReferidoRangoHasta = str_replace("-", "", $_GET['hasta']);
    ...
    $vendedorRow = $con->query("SELECT vn_nombre FROM vendedores WHERE IDVendedor = '$id'")->fetch_assoc();

    $vnReferidosSql = $con->query("SELECT * FROM citas AS ct
    ...
    $tituloDe = !empty($vnReferidoRangoDe) ? ' - desde '.$_GET['de'] : '' ;
    $tituloHasta = !empty($vnReferidoRangoHasta) ? ' - hasta '.$_GET['hasta'] : '' ;
 	$tituloReporte = 'Referidos Vendedor '.$vendedorRow['vn_nombre'].$tituloDe.$tituloHasta;
...
//Se agregan los datos de los alumnos
 ...
 while ($fila = $vnReferidosSql->fetch_assoc()) {

    $fechaVnReferido = $fila['ct_anoCita'].'/'.$fila['ct_mesCita'].'/'.$fila['ct_diaCita'];
     
     $objPHPExcel->setActiveSheetIndex(0)
         ->setCellValue('A'.$i, $fechaVnReferido)
