<?php include'config.php'; $unidadID = $_POST['id']; 
$unidadRow = $con->query("SELECT * FROM unidadesodontologicas WHERE IDUnidadOdontologica = '$unidadID'")->fetch_assoc();
?>
	<link href="css/calendar.min.css" rel="stylesheet">

<div class="modal-header">  
  <a class="close" data-dismiss="modal">&times;</a>  
  <h4 class="modal-title">Horario Unidad: <?php echo $unidadRow['uo_nombre'] ?></h4>
</div>
	...
	$(document).ready(function() {
		...
		$(document).on('click', '.diaSelected', function(){
			...
			var unidadForm = <?php echo $unidadID ?>;			

			var consultoriosId = $(this).attr("id");
		    if(consultoriosId != '' && unidadForm != '')
		    {
		    	$.ajax({
		        	url:"sucursal-unidad-horario-ver.php",
		            method:"POST",  
		            data:{
		            	id:consultoriosId,
		            	unidad:unidadForm
