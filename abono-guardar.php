...
$firma_abono_paciente = $_POST['firma_abono_paciente'];
$firma_abono_usuario = $_POST['firma_abono_usuario'];
...
if($abono>0){
	...
	if($abonoID){
		$query = $con->query("UPDATE abonos SET ab_idUsuario='$sessionIDUsuario', ab_idSucursal='$sucursal', ab_abono='$abono', ab_idFormaPago='$formaPago', ab_idBanco='$banco', ab_cheque='$cheque', ab_comentario='$comentario' WHERE IDAbono = '$abonoID'");
	} else {
		$consecutivo = $con->query("SELECT * FROM abonos WHERE ab_idClinica='$sessionClinica' AND ab_idSucursal = '$sucursal' ORDER BY IDAbono DESC")->fetch_assoc();
		$nuevoConsecutivo = $consecutivo['ab_consecutivo']+1;

		$query = $con->query("INSERT INTO abonos SET ab_idClinica='$sessionClinica', ab_idUsuario='$sessionIDUsuario', ab_idPaciente='$pacienteID', ab_idSucursal='$sucursal', ab_consecutivo='$nuevoConsecutivo', ab_abono='$abono', ab_idFormaPago='$formaPago', ab_idBanco='$banco', ab_cheque='$cheque', ab_comentario='$comentario', ab_firmaPaciente='$firma_abono_paciente', ab_firmaUsuario='$firma_abono_usuario', ab_fechaOrden='$hoyAno$hoyMes$hoyDia', ab_fechaCreacion='$fechaHoy', ab_estado='1'");
	}
...
<?php }
		...
		if($tipo=='pc'){
							...
							$abonosPacienteNum = $con->query("SELECT * FROM abonos WHERE ab_idPaciente='$pacienteID' AND ab_idSucursal>0")->num_rows;
?>
