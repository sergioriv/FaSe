...
/** Se agrega la libreria PHPExcel */
 ...
 $ID = $_GET['id'];
 	...
 	$doctor = $con->query("SELECT IDDoctor, dc_idClinica, dc_nombres FROM doctores WHERE dc_idClinica='$sessionClinica' AND IDDoctor = '$ID'")->fetch_assoc();

 	$resultado = $con->query("SELECT * FROM citas, sucursales, doctores, pacientes, tratamientos WHERE citas.ct_idClinica='$sessionClinica' AND doctores.IDDoctor = '$ID' AND citas.ct_idSucursal = sucursales.IDSucursal AND citas.ct_idDoctor = doctores.IDDoctor AND citas.ct_idPaciente = pacientes.IDPaciente AND citas.ct_idTratamiento = tratamientos.IDTratamiento ORDER BY citas.ct_fechaOrden DESC");

 	$tituloReporte = 'Historial Citas Doctor - '.$doctor['dc_nombres'];
...
$titulosColumnas = array('Estado', 'Fecha de Cita', 'Duracion', 'Sucursal', 'Paciente', 'Tratamiento', 'Terminado', 'Tipo de Cita');
