...
/** Se agrega la libreria PHPExcel */
 ...
 $ID = $_GET['id'];
 	...
 	$sucursal = $con->query("SELECT IDSucursal, sc_idClinica, sc_nombre FROM sucursales WHERE sc_idClinica='$sessionClinica' AND IDSucursal = '$ID'")->fetch_assoc();

 	$resultado = $con->query("SELECT * FROM citas, sucursales, doctores, pacientes, tratamientos WHERE citas.ct_idClinica='$sessionClinica' AND sucursales.IDSucursal = '$ID' AND citas.ct_idSucursal = sucursales.IDSucursal AND citas.ct_idDoctor = doctores.IDDoctor AND citas.ct_idPaciente = pacientes.IDPaciente AND citas.ct_idTratamiento = tratamientos.IDTratamiento ORDER BY citas.ct_fechaOrden DESC");

 	$tituloReporte = 'Historial Citas Sucursal - '.$sucursal['sc_nombre'];
...
$titulosColumnas = array('Cancelada', 'Fecha de Cita', 'Duracion', 'Doctor', 'Paciente', 'Tratamiento', 'Terminado', 'Tipo de Cita');
 ...
 while ($fila = $resultado->fetch_assoc()) {
	...
	if($fila['ct_control']==1){ $tipoCita = 'Primera'; }
     ...
     $objPHPExcel->setActiveSheetIndex(0)
         ...
         ->setCellValue('D'.$i, $fila['dc_nombres'])
         ->setCellValue('E'.$i, $fila['pc_nombres'])
...
$objPHPExcel->getActiveSheet()->setSharedStyle($estiloCentro, "G4:H".($i-1));
