...
$query = $con->query("UPDATE pacientenopatologicos SET pnp_estado='$estado' WHERE IDpacNoPatologico = '$id'");
if($query){
	if($tipo==1){
?>
...
<?php } else {
		$rip = $con->query("SELECT nopatologicos.IDNoPatologico, nopatologicos.np_nombre, pacientenopatologicos.pnp_idPaciente FROM nopatologicos, pacientenopatologicos WHERE pacientenopatologicos.pnp_idNoPatologico = nopatologicos.IDNoPatologico AND pacientenopatologicos.IDpacNoPatologico = '$id'")->fetch_assoc();
?>
		<div class="contenedorAlerta">
			<input type='radio' id='alertError'>
			<label class='alerta error' for='alertError'>
				<div>Se ha eliminado el Antecedente No Patológico <b><?php echo $rip['np_nombre'] ?></b>.<a class="eliminarNoPat deshacer" id="<?php echo $id ?>" es="0" pc="<?php echo $pacienteID ?>" tipo="1">Deshacer</a></div>
				<div class="close">&times;</div>
			</label>
		</div>
