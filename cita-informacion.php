...
$citaSql = $con->query("SELECT pc_nombres, pc_telefonoFijo, pc_telefonoCelular, dc_nombres, sc_nombre, uo_nombre, tr_nombre, tr_idCups, ct_anoCita, ct_mesCita, ct_diaCita, ct_fechaInicio, ct_horaCita, ct_duracion, ct_nota, ct_evolucionada, ct_asistencia, ct_estado
	...
	if( $citaSql['ct_estado']==2){ $citaEstado = ' - Cancelada'; }
		elseif( $citaSql['ct_asistencia']==2){ $citaEstado = ' - Realizada'; }
...
<div class="modal-body divForm">
	...
	<div id="msj-cita-informacion" class="contenedorAlerta"></div>
	...
	<?php
		if( $citaSql['ct_fechaInicio'].str_replace(':','',$citaSql['ct_horaCita'])>=$fechaHoySinEsp.date('Hi') ){ ?>
			...
			?>
					<a class="boton boton-warning btn_cita_info" data-action="0">Cancelar Cita</a>
					<a class="boton boton-primario btn_cita_info" data-action="1">Confirmar Cita</a>
			<?php
	...
	<?php } ?>
		<a class="boton boton-secundario btn_cita_concentimiento">Consentimiento</a>
	</div>
