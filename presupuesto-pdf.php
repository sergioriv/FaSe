    ...
    <page_header>
        <table class="page_header" cellspacing="0">
            ...
            <tr>
            	...
            	<td class="tituloDoc">Presupuesto No.<?= $presupuestoConsecutivo ?></td>
            </tr>
        </table>
    </page_header>
    <page_footer>
            ...
            $totalPresupuesto = 0;
                        ...
                        $fasesSql = $con->query("SELECT * FROM fases WHERE fs_idClinica IN(0,$sessionClinica)");
                            while($fasesRow = $fasesSql->fetch_assoc()){
                                $tratamientosFase = $con->query("SELECT * FROM presupuestotratamientos AS ppt 
                                    INNER JOIN tratamientos AS tr ON ppt.ppt_idTratamiento = tr.IDTratamiento 
                                    WHERE tr_idFase='$fasesRow[IDFase]' AND ppt_idPresupuesto='$presupuestoID'");
                                if($tratamientosFase->num_rows > 0){
                            ?>
                                ...
                                <?php 
                                    while($tratamientosRow = $tratamientosFase->fetch_assoc()){

                                        $subTotal += $tratamientosRow['ppt_precio'];
                                ?>
                                        <tr>
                            ...
                            <?php }
                                ...
                                $totalPresupuesto = $subTotal - ( ( $subTotal * $presupuestoQuery['cnv_descuento'] ) / 100 );

                                $subTotal = '$'.number_format($subTotal, 0, ".", ",");
                                $convenioValor = number_format($presupuestoQuery['cnv_descuento'], 0, ".", ",");
                                $totalPresupuesto = '$'.number_format($totalPresupuesto, 0, ".", ",");

                            ?>

                            <div class="valorTotal">Sub total: <?= $subTotal ?></div>
                            <div class="valorTotal">Convenio <?= $presupuestoQuery['cnv_nombre']?>: <?= $convenioValor ?> %</div>
                            <div class="valorTotal">Total presupuesto: <?= $totalPresupuesto ?></div>
            ...
            <tr>
                <td>
                    <div class="content_signature left">
                        <?php if(!empty($presupuestoQuery['pp_firmaPaciente'])){ ?>
                            <img width="150" src="<?php echo $presupuestoQuery['pp_firmaPaciente'] ?>">
                        <?php } ?>
                ...
                <td>
                    <div class="content_signature right">
                        <?php if(!empty($presupuestoQuery['pp_firmaUsuario'])){ ?>
                            <img width="150" src="<?php echo $presupuestoQuery['pp_firmaUsuario'] ?>">
                        <?php } ?>
